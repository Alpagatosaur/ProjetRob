...
WPSTOP = []
WPlist = [ [-20.0, -20.0],[20.0 , 20.0] , [-20.0 , 20.0] ,[20.0 , -20.0] , [0.0, 0.0]]
...
seuil =0
n = 0
list_add = [[-10 , 0] , [-10 , 10] , [0 , 10] , [10 , 10] , [10 , 0], [10 , -10] , [ 0 , -10] , [-10 , -10]]
...
for t in simu.t: 
    ...
    if (np.sqrt((robot.y-WPManager.yr)**2 + (robot.x-WPManager.xr)**2) < epsilonWP): # robot est dans zone en cercle
        ...
        while (check == 0):
            ...
            elif ( index < len(WPlist)-1):
                
                del WPlist[-1]
                v= point_stop(Listpotential ,Listpoints)
                WPlist.append(v)
                WPlist.append([0.0 , 0.0])
                check = 1
